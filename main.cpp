...
using StateMatrix = std::array<std::array<std::array<double, 3>, 2>, 33>; // I don't even know why I did it this way
...
std::vector<Vec3> propagate(Vec3 p0, Vec3 v0, SpiceDouble date_init, SpiceDouble date_final, double dt)
{
    ...
    const double body_GMs[33] = {
        1.3271244004193938E+11,        //"SUN",
        2.2031780000000021E+04,           //"MERCURY",
        3.2485859200000006E+05,        //"VENUS",
        3.9860043543609598E+05,        //"EARTH",
        4.9028000661637961E+03,        //"MOON",
        4.282837362069909E+04,        //"MARS",
        1.266865349218008E+08,        //"JUPITER",
        5.959916033410404E+03,        //"IO",
        3.202738774922892E+03,        //"EUROPA",
        9.887834453334144E+03,        //"GANYMEDE",
        7.179289361397270E+03,        //"CALLISTO",
        3.793120749865224E+07,        //"SATURN",
        2.503522884661795E+00,        //"MIMAS",
        7.211292085479989E+00,        //"ENCELADUS",
        4.121117207701302E+01,       //"TETHYS",
        7.311635322923193E+01,        //"DIONE",
        1.539422045545342E+02,        //"RHEA",
        8.978138845307376E+03,        //"TITAN",
        3.718791714191668E-01,        //"HYPERION",
        1.205134781724041E+02,        //"IAPETUS",
        5.793951322279009E+06,        //"URANUS",
        8.346344431770477E+01,        //"ARIEL",
        8.509338094489388E+01,        //"UMBRIEL",
        2.269437003741248E+02,         //"TITANIA",
        2.053234302535623E+02,        //"OBERON",
        6.835099502439672E+06,        //"NEPTUNE",
        1.427598140725034E+03,        //"TRITON",
        8.696138177608748E+02,        //"PLUTO",
        1.058799888601881E+02,        //"CHARON",
        6.3130000000000003E+01,        //"CERES",
        1.3730000000000000E+01,        //"PALLAS",
        1.7289999999999999E+01,        //"VESTA",
        1.8100000000000001E+00        //"PSYCHE"
    };
    ...
    for (int cycle = 0; cycle < N_cycles; cycle++)
    {
        ...
        {
            double prop_progress = (double)cycle / (double)N_cycles * 100;
            std::cout << "    Propagating: " << prop_progress << "\%\n";
        }
    }
...
std::tuple<std::vector<std::string>, std::vector<std::string>, std::vector<std::string>,
    ...
    generateEphemeris(SpiceDouble t0, Vec3 p0, Vec3 v0, std::vector<SpiceDouble> ts, double dt = -1)
{
    ...
    for (int idx_tdiff = 0; idx_tdiff < ts.size(); idx_tdiff++)
    {
        if (ts[idx_tdiff] > t0)
        {
            std::vector<Vec3> sv_prop = propagate(p_last, v_last, t_last, ts[idx_tdiff], dt);
            ...
            // get rates
            std::vector<Vec3> sv_prop2 = propagate(p_t, v_t, ts[idx_tdiff], ts[idx_tdiff] + eps_rate, dt);
        ...
        else if (ts[idx_tdiff] == t0) // the date is epoch date
        {
            ...
            // get rates
            std::vector<Vec3> sv_prop2 = propagate(p0, v0, t0, t0 + eps_rate, dt);
        ...
        {
            std::vector<Vec3> sv_prop = backpropagate(p_last, v_last, t_last, ts[idx_tdiff], -dt);
            std::vector<Vec3> sv_apparent = computeApparentStateGeocentric(sv_prop[0], sv_prop[1], ts[idx_tdiff]);
            Vec3 p_t = sv_prop[0];
            Vec3 v_t = sv_prop[1];
            Vec3 p_a = sv_apparent[0];
        ...
        double main_progress = (double)idx_tdiff / (double)ts.size() * 100;
        std::cout << "\nGenerating ephemeris: " << main_progress << "\%\n";
    }
        ...
        double dt = -1)
{
    ...
    for (int idx_tdiff = 0; idx_tdiff < ts.size(); idx_tdiff++)
    {
        if (ts[idx_tdiff] > t0)
        {
            std::vector<Vec3> sv_prop = propagate(p0, v0, t0, ts[idx_tdiff], dt);
        ...
        {
            std::vector<Vec3> sv_prop = backpropagate(p0, v0, t0, ts[idx_tdiff], -dt);
            std::vector<Vec3> sv_apparent = computeApparentState(sv_prop[0], sv_prop[1], ts[idx_tdiff], obscode_map[obscode]);
            Vec3 p_t = sv_prop[0];
            Vec3 v_t = sv_prop[0];
            Vec3 p_a = sv_apparent[0];
...
{
    std::cout << " === HPEP HELP ===\n";
    std::cout << "HPEP is an ephemeris generation software for minor planets and natural satellites in the Solar System.\n";
    std::cout << "It is the high-precision counterpart of MPEP software.\n\n";
    ...
    std::cout << "The minimal invocation is merely 'hpep' - this assumes default locations for all files, which are:\n\n";
    std::cout << "SPICE Folder:\n";
    std::cout << "data/SPICE/\n";;
    std::cout << "Minor Planet Center Observatory Codes:\n";
    std::cout << "data/ObsCodes.dat\n\n";
    std::cout << "HPEP was developed by H. A. Guler.\n\n";
    std::cout << "HPEP is licensed under GNU General Public License version 2.0 (GPL-2.0 License)\n\n";
}

int main(int argc, char* argv[])
{
    std::cout << "HPEP v0.1.0\n\n";
    ...
    for (int idx_cmd = 1; idx_cmd < argc; idx_cmd++)
    {
        ...
        else if (!strcmp(argv[idx_cmd], "-dt") || !strcmp(argv[idx_cmd], "-delta_t"))
        {
            argtype = 11;
        }
        ...
        else
        {
            ...
            case 10:
                ...
                break;
            case 11:
                param_dt = strtod(argv[idx_cmd], NULL);
            }
        }
    }
    ...
    {
        ephem_data = generateEphemeris(t0, p0, v0, ts, param_dt);
        // returns {date, RA, DEC}
    }
    else
    {
        ephem_data = generateEphemerisObsv(t0, p0, v0, ts, user_obscode, obscode_map, param_dt);
        // returns {date, RA, DEC}
    }
